...
A partir dos onze arquivos com as tentativas de transmissão, foi calculado o PDR de cada modulação, para cada um dos dispositivos Tx em cada uma das três transmissões realizadas. Na Tabela \ref{table:pdr} estão detalhados os valores de PDR para cada dispositivo, modulação e transmissão. \refact{Os dispositivos ``D'' e ``H'' foram posicionados no primeiro piso, os dispositivos ``A'', ``B'' e ``C'' foram posicionados no segundo piso, os grupos formados pelos dispositivos ``E'', ``F'' e ``G'' e ``I'', ``J'' e ``K'' foram posicionados no terceiro e quarto piso do prédio, respectivamente.}
...
\todo{explicar melhor os elementos dos gráficos}
